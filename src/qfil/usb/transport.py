...
class QualcommUsbTransport:
    ...
    @classmethod
    def auto(cls, timeout_ms: int = 5000) -> "QualcommUsbTransport":
    ...
    def reopen(self) -> None:
        """Close and reopen the transport to clear stale USB state."""
        self.close()
        time.sleep(0.5)
        self.open()
        self.drain()
    ...
    def drain(self, timeout_ms: int = 1000, max_rounds: int = 30) -> None:
        """Read and discard all pending data from the device, logging any XML content."""
        total = 0
        ...
        if total:
            log.info("Drained %d bytes of pending device data", total)
